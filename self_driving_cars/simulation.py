...
def create_controller_output_dir():
  ...
  return CONTROLLER_OUTPUT_FOLDER


def store_trajectory_plot(graph, fname):
  """ Store the resulting plot.
  """
  CONTROLLER_OUTPUT_FOLDER = create_controller_output_dir()
...
def exec_waypoint_nav_demo(args):
  ...
  with make_carla_client(args.host, args.port) as client:
    ...
    for frame in range(TOTAL_EPISODE_FRAMES):
      ...
      if skip_first_frame and frame == 0:
        ...
        trajectory_fig.update("lookahead_path",
                              new_waypoints_np[path_indices.astype(
                                  int), 0],
                              new_waypoints_np[path_indices.astype(
                                  int), 1],
    ...
    write_trajectory_file(x_history, y_history,
                          speed_history, time_history)
